...
- Use a hosted Supabase cloud project as the primary development and runtime database/auth backend.
- Manage schema changes through Supabase CLI migrations linked to the remote project, not through local Dockerized Supabase services.
...
- Developers must use `supabase login`, `supabase link`, and `supabase db push` carefully because migration mistakes apply to the hosted environment.
